...
QMaybe<QCapturableWindow> QPlatformMediaIntegration::capturableWindowFromQWindow(QWindow *window)
{
    ...
    if (!capturableWindows)
		return { QUnexpect{}, QStringLiteral("No windowcapture platform implementation") };
    if (window == nullptr)
        return { QUnexpect{},  QStringLiteral("QWindow is nullptr") };
    if (!window->isTopLevel())
        return { QUnexpect{},  QStringLiteral("QWindow is not top-level.") };
